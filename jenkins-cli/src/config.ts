...
export const DEFAULT_CFG_REL = ".config/jenkins-cli/config.json";
...
/** 为什么: 配置是唯一连接真相, 缺字段必须失败, 不能默默补一台 Jenkins. */
...
function parseProfile(item: unknown, path: string, index: number): Profile {
  ...
  const username = readStr(raw.username);
  if (username === "") {
    throw new Error(`invalid config file ${path}: username is required for profile ${name}`);
  }
  return {
    name,
    url,
    ...
    apiToken: readStr(raw.apiToken) || readStr(raw["api-token"]),
  };
}
...
/** 为什么: apiToken 优先是用户点名的例外, 两空必须失败, 禁止再猜第三处密钥. */
export function resolveSecret(profile: Profile): string {
  ...
  throw new Error(`profile ${profile.name} has empty apiToken and password`);
}
