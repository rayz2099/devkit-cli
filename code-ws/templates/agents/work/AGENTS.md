...
路由入口只走全局 skill, 禁止模型自行选择阶段. 必须由用户显式推动:

- `$dt-plan`: 拆解任务并建立验证、部署决策. 对应 `planning`.
- `$dt-code`: 逐项目实施, 默认以编译通过为验收门禁. 对应 `coding`.
- `$dt-deploy`: 仅在计划要求或用户明确要求时执行. 对应 `deploy`. 跳过部署的任务可由 `$dt-code` 直接收口.
