...
`agent`, a pipe, or a Kind with no vendor client (`elasticsearch`, later `s3` / `kafka`) cannot enter Console: they error and must use `query`. Exit status of Console is the vendor client's, untranslated.
